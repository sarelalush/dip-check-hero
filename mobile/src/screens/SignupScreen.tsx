...
export function SignupScreen({ navigation }: Props) {
  ...
  return (
    ...
    >
      <AuthField icon="user" label="שם מלא" onChangeText={setName} placeholder="הכנס שם מלא" value={name} />
      <AuthField icon="user" keyboardType="phone-pad" label="טלפון" onChangeText={setPhone} placeholder="050-1234567" value={phone} />
      ...
      <AuthField icon="lock" label="סיסמה" onChangeText={setPassword} placeholder="בחר סיסמה" secure sideIcon="eye" value={password} />

      <AuthMessage text={error} tone="error" />
      <AuthMessage text={success} tone="success" />

      <AuthPrimaryButton busy={busy} disabled={googleBusy} label="הרשמה" onPress={submit} />
      <AuthDivider />
      <SocialButton disabled={busy || googleBusy} label="המשך עם Apple" mark="apple" onPress={() => setError('הרשמה עם Apple תהיה זמינה בקרוב.')} />
...
const styles = StyleSheet.create({
  ...
  accountLink: { color: colors.primary, fontFamily: typography.fontFamilySemiBold, fontSize: 16, fontWeight: '900', ...rtl.text },
});
