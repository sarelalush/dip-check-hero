...
import { ChevronLeft, LogOut, Shield, Bell, HelpCircle, Info, Globe, Ruler, FileText } from "lucide-react";
...
export const Route = createFileRoute("/settings")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "ניהול חשבון, העדפות ועזרה באפליקציה" },
    ],
  }),
  ...
  return (
    <div dir="rtl" className="relative min-h-screen overflow-hidden bg-gradient-to-b from-[#E6F6FB] via-background to-background">
      ...
      <div className="relative mx-auto max-w-md px-5 pt-6">
        ...
        {/* App Settings */}
        <Section title="הגדרות אפליקציה">
          <Row icon={<Bell className="h-5 w-5" />} label="התראות" hint="בקרוב" disabled />
          <Row icon={<Globe className="h-5 w-5" />} label="שפה" value="עברית" disabled />
          <Row icon={<Ruler className="h-5 w-5" />} label="יחידות מידה" value="מטרי" disabled />
        </Section>

        {/* Admin */}
        {isAdmin && (
          <Section title="מנהל">
            <Row icon={<Shield className="h-5 w-5" />} label="פאנל מנהל" to="/admin" />
          </Section>
        )}

        {/* Help & About */}
        <Section title="עזרה ומידע">
          <Row icon={<FileText className="h-5 w-5" />} label="איך משתמשים?" to="/welcome" />
          <Row icon={<HelpCircle className="h-5 w-5" />} label="שאלות נפוצות" hint="בקרוב" disabled />
          <Row icon={<Info className="h-5 w-5" />} label="אודות AquaSense" disabled />
        </Section>

        {/* About footer inside card feel */}
        <div className="mt-6 overflow-hidden rounded-[22px] bg-card p-5 shadow-[var(--shadow-card)] text-center">
          <div className="text-sm font-black text-foreground">AquaSense</div>
          <div className="mt-1 text-xs font-semibold text-muted-foreground">גרסה 1.0</div>
          <p className="mt-3 text-[11px] font-medium text-muted-foreground leading-relaxed">
            AquaSense עוזרת לך לשמור על בריכה נקייה ובטוחה בצורה פשוטה וחכמה — סריקת מדבקות צבע, המלצות תיקון ומעקב היסטורי.
          </p>
          <div className="mt-3 text-[10px] font-bold text-muted-foreground/60">
            2025 AquaSense. כל הזכויות שמורות.
          </div>
        </div>
...
function Row({
  icon, label, to, hint, value, disabled,
}: { icon: React.ReactNode; label: string; to?: string; hint?: string; value?: string; disabled?: boolean }) {
  const content = (
    <div className={`flex items-center justify-between gap-3 px-4 py-3.5 border-b border-border/40 last:border-b-0 ${disabled ? "opacity-60" : "active:bg-muted/40"}`}>
      ...
      <div className="flex flex-1 items-center justify-end gap-3">
        ...
        {value && <span className="text-xs font-bold text-muted-foreground">{value}</span>}
